...
class List_pasien extends MY_Controller
{
	...
	public function instansi()
	{
		...
		$result = $this->reg->get_list_instansi($params);
		// echo "<pre>";
		// var_dump($result);exit();
		...
		$parse = array(
			...
			'main_menu' => 'list',
			'sub1' => 'listinstansi',
			'content' => 'content/list_pasien/list_instansi',
			'page_list' => $result,
			'input' => $params,
		);
	...
	public function instansi_table_part()
	{
		$params = $this->input_sanitizer->method('get')
			->date('from_date', ['d/m/Y', 'Y-m-d'])
			->date('to_date', ['d/m/Y', 'Y-m-d'])
		...
		$base_view = 'content/list_pasien/list_instansi';
