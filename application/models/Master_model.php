...
class Master_model extends CI_Model {
	...
	public function get_negara($id='')
	{
		if ($id!='') {
			$this->sv->where('id_negara',$id);
		}
		$neg = $this->sv->get('master_negara');
		return $neg->result();
	}
	...
	public function get_instansi_unit($id='')
	{
		if ($id!='') {
	...
	public function get_instansi($id='')
	{
		...
		$stat = $this->sv->get('TBLInstansi');
		return $stat->result();
	}

	public function get_tindakan_micro($id=13)
	{
		$this->sv->where('KdGroup', $id);
	...
	function getUnitInstansi($val){
		if ($val!='') {
			$this->sv->where('KdInstansi', $val);
		}
		$stat = $this->sv->get('TBLUnitInstansi');
		return $stat->result();
	...
	function getTarifTindakan($val){
		$this->sv->where('KDDetail', $val);
		$this->sv->where('Kategori', 1);
		$this->sv->where('KdKelas', 'rj');	
		$stat = $this->sv->get('fTarifPatologi');
		return $stat->row();
	...
	function get_list_per_instansi($val){
		$this->sv->where('regno_instansi', $val);	
		$stat = $this->sv->get('Register');
		return $stat->result();
	}

	public function negara_post($data)
	{
		$cek = $this->sv->where('nm_negara', $data['nm_negara'])->get('master_negara')->row();
		if (empty($cek)) {
			$last = $this->sv->order_by('id_negara','DESC')->limit(1)->get('master_negara')->row();
			$last = $last->id_negara+1;
			$input = $this->sv->insert('master_negara',['id_negara'=> $last, 'nm_negara'=>$data['nm_negara']]);
			if ($input) {
				$parse = array(
					'cek' => $cek,
					'insert' => $input,
			...
			$parse = array(
					'cek' => $cek,
					'insert' => FALSE,
					'message' => 'Nama Negara Sudah Ada, silahkan hapus yang ada.'
				);
			return $parse;
		}
	...
	public function negara_delete($id_negara)
	{
	
		$delete = $this->sv->delete('master_negara', array('id_negara' => $id_negara));
		if ($delete) {
			$parse = array(
				'delete' => $delete,
