...
class Master_tarif extends CI_Model {
	...
	public function tarif_laboratorium($kode='')
	{
		...
		$this->sv->select('t.*, p.NMDetail, r.NmBangsal, kat.NmKategori');
		$stat = $this->sv->from('fTarifPatologi t')->join('fPemeriksaanPatologi p', 'p.KDDetail = t.KDDetail', 'left')->join('TBLKategoriPsn kat', 'kat.KdKategori = t.Kategori', 'left')->join('TBLBangsal r', 'right(t.KDKelas,2) = r.KdBangsal', 'left')->get();
		return $stat->result();	
	}
	...
	public function tarif_lab_post($data)
	{
		...
		if (!empty($listing)) {
			...
			$input = $this->sv->insert('fTarifPatologi',[
	...
	{
		$update = $this->sv->where('KDTarif',$data['kdtarif'])->update("fTarifPatologi",[
	...
	public function input_tarif_excel___()
	{
		...
		foreach ($kategori as $kdkategori) {
			foreach ($kelas as $kdkelas) {
				...
				$cek_tarif = $this->sv->select("*")->from('fTarifPatologi')->where('KDTarif', $kddetail.$kdkategori->KdKategori.$kdkelas->KDKelas)->get()->row();
				if (empty($cek_tarif)) {
					$input = $this->sv->insert('fTarifPatologi', $data);
					if ($input) {
						$inputan++;	
					}
				} else {
	...
	{
		$search_tarif = $this->sv->select('*')->from('fTarifPatologi')->where('KDDetail', $kddetail)->get()->result();
		if (!empty($search_tarif)) {
			$delete_tarif = $this->sv->where('KDDetail', $kddetail)->delete('fTarifPatologi');
		}
		...
		$delete_pemeriksaan = $this->sv->where('KDDetail', $kddetail)->delete('fPemeriksaanPatologi');
	...
	{
		$delete_group = $this->sv->where('KDGroup', $kode)->delete('fGroupPatologi');
		if ($delete_group) {
			$parse = array(
				'status' => true,
				'message' => 'Berhasil dihapus');
