...
<?php else: ?>
  <?php foreach ($data as $key => $l): ?>
    <?php if($l->HasFilledIn == 0): ?>
      <tr style="background: #CC3338; color: white;">
    <?php elseif($l->HasNotFilled == 1 && $l->HasFilledIn == 1): ?>
    ...
    <?php endif; ?>
      ...
      <td><?= $l->PrintCount ?? '0' ?>x</td>
      <td>
        <a href="<?= base_url('hasilpemeriksaan/show_pemeriksaan/'.$l->Notran) ?>" title="Isi Pemeriksaan Patologi Anatomi">
        ...
        <a href="<?= base_url('hasilpemeriksaan/print_bukti_pengambilan?notransaksi='.$l->Notran) ?>" target="_blank" title="Print Bukti Pengambilan Hasil Patologi Anatomi">
          <i class="fa fa-print purple"></i>
        </a>
        <a href="<?= base_url('hasilpemeriksaan/print_hasil_pemeriksaan_micro?notransaksi='.$l->Notran) ?>" title="Print Hasil Pemeriksaan" target="_blank">
